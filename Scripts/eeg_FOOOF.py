...
with open('fooof_results.txt', 'w') as f:
    f.write("Subject\tsegment\tepoch\tROI\tOffset\tExponent\tR2\tMAE\tResidual_Delta\tResidual_Theta\tResidual_Alpha\tResidual_Beta\tDelta\tTheta\tAlpha\tBeta\t\n")
...
for subject in subjectlist:
    ...
    for s in range(0, len(segment)):
        endtime = eventdf[eventdf['Code'] == segmentcode[s]]['trigger_time'].values[0]
        ...
        for roi in roi_channels:
            ...
            for epoch in range(1, 21):
                data = filterdata[endtime - (epoch * 60 * 500):endtime - ((epoch-1) * 60 * 500)]
                ...
                df.to_csv('D:\\restingstate\\export_eeg_psd\\min\\' + subject + '_' + roi + '_' + str(epoch) + '.csv', index=False)
                ...
                df_aperiod.to_csv('D:\\restingstate\\export_eeg_aperiod\\min\\' + subject + '_' + roi + '_' + str(epoch) + '.csv', index=False)
                ...
                for band_name, (fmin, fmax) in bands.items():
                    ...
                    print(f"{band_name} FOOOF periodic power: {band_power:.4e} µV²/Hz")
                    with open('fooof_results.txt', 'a') as f:
                ...
                for band_name, (fmin, fmax) in bands.items():
                    ...
                    print(f"{band_name} raw PSD power: {band_power:.4e} µV²/Hz")
                    with open('fooof_results.txt', 'a') as f:
                        f.write(str(band_power) + '\t')
